    ...
    <tbody>
        @if(count($user_type) > 0)
            ...
            <tr>
                ...
                @else
                    <td>                      
                        <button type="button" class="btn btn-sm btn-outline-primary btn-flat" id="manage" data-add="{{$user_types->user_type_id}}||{{$user_types->type_name}}" {{$edit}}><i class="fa fa-info"></i> Manage Access</button>               
                    </td>
                @endif
                @if($user_types->created_by == "default")
                    <td>
                        <button class="btn btn-sm btn-outline-info btn-flat" id="edit_usertype" data-add="{{$user_types->user_type_id}}]]{{$user_types->type_name}}]]{{$user_types->type_description}}" disabled><i class="fa fa-edit"></i> Edit</button>
                        <button class="btn btn-sm btn-outline-danger btn-flat" id="delete_usertype" data-add="{{$user_types->user_type_id}}]]{{$user_types->type_name}}" disabled><i class="fa fa-trash"></i> Delete</button>                       
                    </td>
                @else
                    <td>
